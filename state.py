...
def render_snakes(snakes: Iterable[Snake]) -> NDArray[np.uint8]:
    canvas = np.zeros((BOARD_H, BOARD_W, 4), dtype=np.uint8)

    for s in snakes:
        pos = s.positions()
        if isinstance(pos, tuple) and len(pos) == 2:
            r_all = np.asarray(pos[0], dtype=np.int32).ravel()
            c_all = np.asarray(pos[1], dtype=np.int32).ravel()
        else:
        ...
        m = _clip_mask(r_all, c_all, BOARD_H, BOARD_W)
        if not np.any(m):
            continue
        r = r_all[m]; c = c_all[m]
...
def render_food(foods: Iterable[Food], canvas: NDArray[np.uint8] = None) -> NDArray[np.uint8]:
    H = W = BOARD_H
    if canvas is None:
        canvas = np.zeros((H, W, 4), dtype=np.uint8)

    # Collect all food positions at once for vectorized draw
    if not foods:
        return canvas
...
def run_rendering_thread():
    ...
    def _loop():
        ...
        while not stop_event.is_set():
            t0 = time()
            remove_expired_food(now=t0)
            add_food(new_n=FOOD_PER_FRAME)
            move_snakes()
            resolve_collisions()

            with STATE_LOCK:
                snakes_snapshot = deepcopy(list(ACTIVE_SNAKES.values()))
                food_snapshot = deepcopy(list(ACTIVE_FOOD.values()))

            frame = render_snakes(snakes_snapshot)
            frame = render_food(food_snapshot, frame)
            LAST_FRAME = frame
            LAST_RENDER = time()
            ...
            # measure the *real* frame time including sleep and update observed FPS
            loop_dt = time() - t0
            if loop_dt > 0.0:
                momentum = 0.0 if OBSERVED_FRAME_RATE == 0.0 else 0.9
                OBSERVED_FRAME_RATE = 0.9 * OBSERVED_FRAME_RATE + (1.0 - momentum) / loop_dt
